...
def upgrade() -> None:
    """Upgrade schema."""
    logger.info(
        f"Starting Rename response_file_id to response_file_message_ts in questions table upgrade"
    )
    ...
    logger.info(
        f"Completed Rename response_file_id to response_file_message_ts in questions table upgrade"
    )
...
def downgrade() -> None:
    """Downgrade schema."""
    logger.info(
        f"Starting Rename response_file_id to response_file_message_ts in questions table downgrade"
    )

    # ### commands auto generated by Alembic - please adjust! ###
    op.add_column(
        "questions",
        sa.Column("response_file_id", sa.VARCHAR(), autoincrement=False, nullable=True),
    )
    op.drop_index(op.f("ix_questions_response_file_message_ts"), table_name="questions")
    op.create_index(
        "ix_questions_response_file_id", "questions", ["response_file_id"], unique=False
    )
    op.drop_column("questions", "response_file_message_ts")
    op.create_table(
        "checkpoint_writes",
    ...
    logger.info(
        f"Completed Rename response_file_id to response_file_message_ts in questions table downgrade"
    )
