"""FastAPI server for the ragstar."""
...
from ragstar.core.dbt_parser import DBTProjectParser
...
from ragstar.utils.config import load_config
from ragstar.utils.cli_utils import load_dotenv_once
from ragstar.core.models import DBTModel, Question, ModelEmbedding
from ragstar.llm.client import LLMClient, is_openai_api_key_valid
from ragstar.core.agent import Agent
from ragstar.parsers.dbt_manifest_parser import DBTManifestParser
from ragstar.parser.dbt_query_parser import parse_sql
from ragstar.utils.logging import get_logger

# Backward compatibility imports
from ragstar.storage import (
    PostgresStorage,
    PostgresVectorStore,
...
app = FastAPI(
    title="ragstar API",
    description="API for interacting with the ragstar",
    version="0.1.0",
)
...
async def root():
    ...
    return {"message": "ragstar API is running"}
...
def start_server(host: str = "0.0.0.0", port: int = 8000):
    ...
    uvicorn.run("ragstar.api.server:app", host=host, port=port, reload=True)
